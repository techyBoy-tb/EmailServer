...
function validateBody(body, response) {
    ...
    if (body.from === '') {
        response.json(40, { response: 'The return email address cannot be empty' });
        return false;
    } else if (body.text === '') {
        response.json(400, { response: 'The message cannot be empty' });
        return false;
    } else if (body.subject === '') {
        response.json(400, { response: 'The subject cannot be empty' });
        return false;
    } else if (body.to === '') {
        response.json(40, { response: 'The recipient email address cannot be empty' });
        return false;
    }
    ...
    if (!EMAIL_REGEX.test(body.from)) {
        ...
        return false;
    } else if (!EMAIL_REGEX.test(body.to)) {
        response.status(400).send('The recipient email address does not match the standard format');
        return false;
    } else if (!EMAIL_REGEX.test(body.to)) {
        response.status(400).send('The recipient email address does not match the standard format');
        return false;
